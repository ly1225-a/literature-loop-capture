...
def write_project_root_docs(project_dir: Path, run_dir: Path, subquestions: list[str]) -> None:
    purpose = [
        "# Literature Evidence Wiki",
        "",
        f"Project export generated from `{run_dir.name}`.",
        "",
        "Research objective: preserve captured articles, reading notes, figures, tables, important seeds, recommended references, coverage decisions, and query-iteration reasoning as traceable literature evidence.",
        "",
...
def export_llm_wiki_project(run_dir: Path, output_dir: Path | None = None, project_name: str = "literature-review") -> Path:
...
def main() -> int:
    ...
    parser.add_argument("--project-name", default="literature-review")
    parser.add_argument(
        "--raw-sources",
        action="store_true",
